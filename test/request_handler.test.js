...
describe('CloudflareRequestHandler', () => {
  ...
  it('parses JSON response when content-type is application/json', async () => {
    const mockResponse = {
      ...
      ok: true,
      headers: {
        get: (k) => (k.toLowerCase() === 'content-type' ? 'application/json; charset=utf-8' : undefined),
    ...
    const { responsePromise } = rh.makeRequest('https://example.test/api', { Accept: 'application/json' }, 'GET');
  ...
  it('parses text response when content-type is text/plain', async () => {
    const mockResponse = {
      ...
      ok: true,
      headers: {
        get: (k) => (k.toLowerCase() === 'content-type' ? 'text/plain; charset=utf-8' : undefined),
    ...
    const rh = new CloudflareRequestHandler();
    const { responsePromise } = rh.makeRequest('https://example.test/text', {}, 'GET');
  ...
  it('returns an aborted error when request is aborted', async () => {
    ...
    const rh = new CloudflareRequestHandler();
    const { responsePromise, abort } = rh.makeRequest('https://example.test/abort', {}, 'GET');
  ...
  it('sends string body for POST and sets Content-Type when string provided', async () => {
    ...
    const rh = new CloudflareRequestHandler();
    const { responsePromise } = rh.makeRequest('https://example.test/post', { 'X-Test': '1' }, 'POST', 'raw body');
    ...
    const contentType = receivedInit.headers.get('Content-Type') || receivedInit.headers.get('content-type');
